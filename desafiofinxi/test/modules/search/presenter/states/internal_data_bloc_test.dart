...
final usecase = SaveSearchGifByTextMock();
final bloc = InternalDataBloc(usecase);
...
main(){
  test("Deve retornar os estados em ordem", (){
    when(usecase.postGif(any)).thenAnswer((realInvocation) async => Right(realInvocation.hashCode));
  ...
  test("Deve retornar os estados em ordem e retornar um ErrorState", (){
    when(usecase.postGif(any)).thenAnswer((realInvocation) async => Left(InvalidEntityError()));
    expect(bloc, emitsInOrder([
      isA<LoadingState>(),
      isA<ErrorState>(),
    ]));
