...
class GiphyDatasource implements IGifSearchDatasource {
  ...
  Future<List<GifModel>> gifSearch(
      String searchText, int itensPerPage, int indexItem) async {
    final response = await dio.get(
        "${Settings.baseUrlPrefix}?api_key=${Settings.giphyApiKey}&q=$searchText}&limit=$itensPerPage&offset=$indexItem");
    return response.statusCode == 200
        ? (response.data['data'] as List)
            .map((map) => GifModel.fromMap(map))
            .toList()
